...
public class TestOnlineSocialNetwork {
	...
	public void testMethodeCallExpressionForInactiveMembers() {
		...
		uutPlatform.deleteInactives(); // Stuart is inactive and should be
										// deleted
		Assert.assertEquals(membersCountBeforeCleaning - 1, uutPlatform.getMember().size()); // is
																								// Stuart
																								// deleted?
		Member stuart = uutPlatform.getMemberForName("Stuart Bloom"); // try to
																		// get
																		// Stuart
		...
		stuart = uutPlatform.getMemberForName("Stuart Bloom"); // try to get
																// Stuart
		Assert.assertNotNull(stuart); // Stuart get?
		stuart.addFriend(uutPlatform.getMemberForName("Howard Wolowitz")); // Stuart
																			// is
																			// adding
																			// a
																			// new
	...
	public void testMethodCallExpressionForBots() {
		...
		int membersCountBeforeCleaning = uutPlatform.getMember().size();
		Member sheldon = uutPlatform.getMemberForName("Dr Dr Sheldon Cooper");// get
																				// Sheldon
		int likesForSheldon = sheldon.countLikesFromContent(); // How many likes
																// has Sheldon?
		...
		Assert.assertEquals(membersCountBeforeCleaning - 1, uutPlatform.getMember().size()); // is
																								// Shelbot
																								// deleted
		Member shelbot = uutPlatform.getMemberForName("Shelbot"); // should not
																	// exist
		assertNull(shelbot);
		likesForSheldon = sheldon.countLikesFromContent(); // How many likes has
															// Sheldon?
		assertEquals(0, likesForSheldon); // Has Sheldon use a Bot to like his
											// stuff?

		// retesting Bots and try to configure shelbot, that he doesn't be found
		// by heuristics
	...
	private void setup() {
		...
		// Initialize package
		set.getPackageRegistry().put(OnlineSocialNetworkPackage.eNS_URI, OnlineSocialNetworkPackage.eINSTANCE);
		eMoflonEMFUtil.installCrossReferencers(set);
		Resource r = set.getResource(eMoflonEMFUtil.createFileURI("resources/onlineSocialNetwork/tbbtTest.xmi", true),
				true);
		uutPlatform = Platform.class.cast(r.getContents().get(0));
