...
/**
 * This class provides project specific editor configuration for text highlighting and templates.
 * Its generated methods are to be filled and invoked during runtime by MOFLON Eclipse plugin
 * in order to achieve a custom text editor functionality.
 */
public class ProcessCodeAdapterTextEditorConfiguration extends MoflonTextEditorConfiguration {
  ...
  public void setKeyWords() {
    addKeyWord("task").as(COLORS.RED);
    addKeyWord("run").as(COLORS.BLUE);
    addKeyWord("import").as(COLORS.GREEN);
  }
  ...
  /**
   * Here, start and end strings of special text ranges defined which are
   * to be highlighted completely. For example, a javadoc starts with "/*" and ends
   * with "*"+"/" and is highlighted completely with blue color.
   */
  public void highlightSequence() {
	...
	public Collection<MoflonEditorTemplate> getTemplates() {
		...
		if (getWord(-1).equals("run")) {
			MoflonEditorTemplate template1 = new MoflonEditorTemplate(
					"invocation", "invoke another task",
					"${invokedTask}::run()", 0);
			templates.add(template1);
		} else if (getWord(-1).equals("task")) {
			MoflonEditorTemplate template2 = new MoflonEditorTemplate("task",
    ...
    /**
   * User defines with this method an one-to-one mapping between model
   * files (.xmi) and text files (opened with text editor). The absolute
   * paths of the files should be put into returned HashMap.
   * You can use abs(path) function to translate a project relative path
   * to an absolute path
   */
  ...
  /**
   * This method is called when a text file has been changed of which
   * absolute path is a contained value in HashMap returned by getModelPathesToTextPathes().
   * The argument is the absolute path of the changed text file.
   * With the help of calling getModelPath(textFilePath)) the path of the corresponding
   * model file can also be accessed. 
   * Please don't use any project relative path in this method. (use abs(projectRelativePath))
   */
  ...
  /**
   * This method is called when a model file has changed of which absolute path
   * is a contained key in the HashMap returned by getModelPathesToTextPathes().
   * The argument is the absolute path of the changed model file.
   * Please don't use any project relative path in this method. (use abs(projectRelativePath))
   */
